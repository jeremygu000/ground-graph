"""LangGraph ingestion workflow (plan.md §6 + M3 tasks).

Single-node workflow: ingest.
IngestionService handles all acquisition, parsing, chunking, and outbox emission
within the same logical operation (transaction boundary depends on shared session).
"""

from __future__ import annotations

from dataclasses import dataclass, field
from typing import Any
from uuid import UUID

from langgraph.graph import END, START, StateGraph

from groundgraph.application.ingestion.services import IngestionService


@dataclass
class IngestionState:
    source_id: UUID | None = None
    file_path: str | None = None
    media_type: str = "text/plain"

    document_id: UUID | None = None
    version_id: UUID | None = None
    created_new_version: bool = False

    error: str | None = None

    telemetry: dict[str, Any] = field(default_factory=dict)


class IngestionWorkflow:
    def __init__(
        self,
        ingestion_service: IngestionService,
    ) -> None:
        self._ingestion = ingestion_service
        self._graph = self._build_graph()

    def _build_graph(self) -> Any:
        g = StateGraph(IngestionState)
        g.add_node("ingest", self._ingest)
        g.add_edge(START, "ingest")
        g.add_edge("ingest", END)
        return g.compile()

    async def _ingest(self, state: IngestionState) -> dict[str, Any]:
        if state.source_id is None:
            return {"error": "source_id is required"}
        if not state.file_path:
            return {"error": "file_path is required"}

        try:
            result = await self._ingestion.ingest_file(
                source_id=state.source_id,
                file_path=state.file_path,
                media_type=state.media_type,
            )
        except Exception as exc:
            return {"error": str(exc)}
        else:
            return {
                "document_id": result.document_id,
                "version_id": result.version_id,
                "created_new_version": result.created_new_version,
            }

    async def run(self, state: IngestionState) -> IngestionState:
        result = await self._graph.ainvoke(state)
        return IngestionState(**result) if isinstance(result, dict) else result
